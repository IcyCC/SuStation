...
def get_landmark(image_url):
    ...
    #     y = item[1]['y']
    #     label = str(item[0])
    return get_orgin(image_url=image_url)['faces'][0]['landmark']


def plot_landmark(image_url):
    ## draw a picture which have a face and landmark
    plt.figure()
    plt.imshow(plt.imread("image_url"))
    for item in get_landmark(image_url).items():
        x = item[1]['x']
        y = item[1]['y']
        label = str(item[0])
        plt.scatter(x, y)
        plt.text(x, y,str(label),fontsize = 15,verticalalignment='center',color='red',fontweight= 'demi')
    plt.show()
